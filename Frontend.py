...
def Draw_pieces(Win, Small_Cross, Small_Circle,Cross, Circle, Small_Square, Square, board, big_board, player):
    for x1 in range(len(board)):
        for y1 in range(len(board)):
            if board[y1][x1] == -1:
                if x1 > 2 and x1 < 6:
                    Win.blit(Small_Circle, (x1*Small_Square+10, y1*Small_Square))

                elif x1 > 5:
                    Win.blit(Small_Circle, (x1*Small_Square+20, y1*Small_Square))
